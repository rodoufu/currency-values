...
public class BitcoinPriceJobTest {
	...
	public void oneFiatPrice() {
		...
		Assert.assertTrue(new BigDecimal(1).subtract(bitcoinPrices.get(0).getValue()).compareTo(error) < 0);
	}
	...
	public void twoFiatPrices() {
		...
		Assert.assertTrue(new BigDecimal(500).subtract(bitcoinPrices.get(0).getValue()).compareTo(error) < 0);
	}
